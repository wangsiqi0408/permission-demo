...
<body class="no-skin" youdao="bind" style="background: white">
    ...
    <script>
        ...
        var aclModuleMap = {}; // 存储map格式的权限列表信息
        ...
        function recursiveRenderAclModule(aclModuleList) {
            if(aclModuleList && aclModuleList.length > 0) {
                $(aclModuleList).each(function (i, item) {
                    aclModuleMap[item.id] = item;
                    if(item.aclModuleList && item.aclModuleList.length > 0) {
                        var rendered = Mustache.render(aclModuleListTemplate, {
                            aclModuleList: item.aclModuleList,
                            "showDownAngle": function () {
                                return function (text, render) {
                                    return (this.aclModuleList && this.aclModuleList.length > 0) ? "" : "hidden";
                                }
                        ...
                        $("#aclModule_" + item.id).append(rendered);
                        recursiveRenderAclModule(item.aclModuleList);
                    }
                })
            }
        }
        ...
        $(".aclModule-add").click(function () {
            $("#dialog-aclModule-form").dialog({
                ...
                buttons: {
                    "确定": function (e) {
                        e.preventDefault();
                        saveAclModule(true, function (data) {
        ...
        function recursiveRenderAclModuleSelect(aclModuleList, level) {
            ...
            if(aclModuleList && aclModuleList.length > 0) {
                $(aclModuleList).each(function (i, item) {
                    ...
                    optionStr += Mustache.render("<option value='{{id}}'>&nbsp;&nbsp;&nbsp;&nbsp;{{name}}</option>", {id: item.id, name: blank + item.name})
                    if(item.aclModuleList && item.aclModuleList.length > 0) {
                        recursiveRenderAclModuleSelect(item.aclModuleList, level + 1);
                    }
                })
            }
        }
        
        function saveAclModule(isCreate, successCallback, failCallback) {
            $.ajax({
            ...
            $(".aclModule-edit").click(function (e) {
                ...
                $("#dialog-aclModule-form").dialog({
                    ...
                    buttons: {
                        "确定": function (e) {
                            e.preventDefault();
                            saveAclModule(false, function (data) {
                                $("#dialog-aclModule-form").dialog("close");
                            }, function (data) {
            ...
            if(result.status) {
                if(result.data.total > 0) {
                    var rendered = Mustache.render(aclListTemplate, {
                        ...
                        "showType": function () {
                            return this.type == 1 ? "菜单" : (this.type == 2 ? "按钮" : "其它");
                        },
                        "bold": function () {
                            return function (text, render) {
                ...
                showMessage("获取权限列表", result.message, false);
            }
        }
        ...
        function saveAcl(isCreate, successCallback, failCallback) {
            $.ajax({
