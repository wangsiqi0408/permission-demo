...
<body class="no-skin" youdao="bind" style="background: white">
    ...
    <script>
            ...
            if(deptList && deptList.length > 0) {
                $(deptList).each(function (i, item) {
                    ...
                    if(item.deptList.length > 0) {
                        var rendered = Mustache.render(deptListTemplate, {deptList: item.deptList});
                        $("#dept_" + item.id).append(rendered);
                        recursiveRenderDept(item.deptList);
                    }
                })
            }
        }
            ...
            $(".dept-edit").click(function (e) {
                ...
                $("#dialog-dept-form").dialog({
                    ...
                    buttons: {
                        "确定": function (e) {
                            e.preventDefault();
                            saveDept(false, function (data) {
                                $("#dialog-dept-form").dialog("close");
                            }, function (data) {
                                showMessage("编辑部门", data.message, false);
                            })
                        },
                        "取消": function () {
                            $("#dialog-dept-form").dialog("close");
            ...
            if(deptList && deptList.length > 0) {
                $(deptList).each(function (i, item) {
                    ...
                    if(item.deptList && item.deptList.length > 0) {
                        recursiveRenderDeptSelect(item.deptList, level + 1);
                    }
                })
            }
        }
        
        function saveDept(isCreate, successCallback, failCallback) {
            $.ajax({
        ...
        function saveUser(isCreate, successCallback, failCallback) {
            $.ajax({
            ...
            $(".user-edit").click(function (e) {
                ...
                $("#dialog-user-form").dialog({
                    ...
                    buttons: {
                        "确定": function (e) {
                            e.preventDefault();
                            saveUser(false, function (data) {
                                $("#dialog-user-form").dialog("close");
                                loadUserList(lastClickDeptId);
                            }, function (data) {
                                showMessage("编辑用户", data.message, false);
                            })
                        },
                        "取消": function () {
                            $("#dialog-user-form").dialog("close");
