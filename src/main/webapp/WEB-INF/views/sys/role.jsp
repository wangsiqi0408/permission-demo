...
<head>
    <title>角色</title>
    <jsp:include page="/WEB-INF/views/common/backend_common.jsp" />
...
<body class="no-skin" youdao="bind" style="background: white">
    ...
    <script>
            ...
            $(".role-edit").click(function (e) {
                ...
                $("#dialog-role-form").dialog({
                    ...
                    buttons: {
                        "确定": function (e) {
                            e.preventDefault();
                            saveRole(false, function (data) {
                                $("#dialog-role-form").dialog("close");
                            }, function (data) {
                                showMessage("编辑角色", data.message, false);
                            })
                        },
                        "取消": function () {
                            $("#dialog-role-form").dialog("close");
        ...
        function saveRole(isCreate, successCallback, failCallback) {
            $.ajax({
        ...
        function handleRoleSelected(roleId) {
            ...
            if(selectFirstTab) {
                //加载角色权限列表
                loadRoleAclList(roleId);
            }
        }

        function loadRoleAclList(selectedId) {
            if(selectedId == -1) {
                return;
            }
            $.ajax({
        ...
        function recursivePrepareTreeData(aclModuleList) {
            ...
            if (aclModuleList && aclModuleList.length > 0) {
                $(aclModuleList).each(function(i, aclModule) {
                    ...
                    if ((aclModule.aclModuleList && aclModule.aclModuleList.length > 0) || (aclModule.aclList && aclModule.aclList.length > 0)) {
        ...
        $(".saveRoleAcl").click(function (e) {
            e.preventDefault();
            if(lastClickRoleId == -1) {
                showMessage("保存角色与权限点的关系", "请先在左侧选择要操作的角色", false);
                return;
            }
            $.ajax({
                type: "POST",
        ...
        $("#roleTab a[data-toggle='tab']").on("shown.bs.tab", function (e) {
            if(lastClickRoleId == -1){
                showMessage("加载角色关系", "请先在左侧选择要操作的角色", false);
                return;
            }
            if(e.target.getAttribute("href") == '#roleAclTab') {
                selectFirstTab = true;
                loadRoleAclList(lastClickRoleId);
            }else {
                selectFirstTab = false;
                loadRoleUserList(lastClickRoleId);
            }
        })

        function loadRoleUserList(selectedRoleId) {
            $.ajax({
                type: "POST",
                url: "/sys/role/userList.json",
                data: {roleId: selectedRoleId},
                success: function (result) {
                    if(result.status) {
                        var renderedSelected = Mustache.render(selectedUsersTemplate, {userList: result.data.selected});
                        var renderedUnselected = Mustache.render(unSelectedUsersTemplate, {userList: result.data.unselected});
        ...
        $(".saveRoleUser").click(function (e) {
            e.preventDefault();
            if(lastClickRoleId == -1) {
                showMessage("保存角色与用户的关系", "请先在左侧选择要操作的角色", false);
                return;
            }
            $.ajax({
                type: "POST",
